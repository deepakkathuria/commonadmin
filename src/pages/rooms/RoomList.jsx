...
const BASE_URL = PROD_URL; // Change to PROD_URL for production
...
export default function RoomList() {
  ...
  const fetchRooms = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/available-room-types`);
      setRooms(res.data.rooms);
    } catch (error) {
      console.error('Error fetching rooms:', error);
    }
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`${BASE_URL}/api/rooms/${currentRoom.id}`, {
        room_type: currentRoom.room_type,
        base_price: currentRoom.base_price,
        total_inventory: parseInt(currentRoom.total_inventory),
        max_adults: parseInt(currentRoom.max_adults),
        max_children: parseInt(currentRoom.max_children),
        addons: currentRoom.addons
          ? currentRoom.addons.split(',').map((a) => a.trim())
          : []
      });
          ...
          <TableBody>
            {rooms.map((room) => (
              <TableRow key={room.id}>
                ...
                <TableCell>₹{parseFloat(room.base_price).toFixed(2)}</TableCell>
                <TableCell>{room.total_inventory}</TableCell>
                <TableCell>{room.max_adults}</TableCell>
                <TableCell>{room.max_children}</TableCell>
                <TableCell>{room.addons?.join(', ')}</TableCell>
                <TableCell>
                  <IconButton color="primary" onClick={() => openEditModal(room)}>
                    <EditIcon />
        ...
        <Box sx={modalStyle}>
          ...
          <Stack spacing={2}>
            ...
            <TextField
              ...
              fullWidth
              type="number"
              value={currentRoom?.max_adults || ''}
              onChange={(e) =>
                setCurrentRoom({ ...currentRoom, max_adults: e.target.value })
              }
            />
            <TextField
              label="Max Children"
              fullWidth
              type="number"
              value={currentRoom?.max_children || ''}
              onChange={(e) =>
                setCurrentRoom({ ...currentRoom, max_children: e.target.value })
              }
            />
            <TextField
              label="Addons (comma-separated)"
